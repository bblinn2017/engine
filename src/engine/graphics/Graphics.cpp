...
void Graphics::addShape(const std::string &name, const std::vector<float> &positions, const std::vector<float> &normals, const std::vector<float> &texCoords){
    // Create shape
    std::shared_ptr<Shape> shape = std::make_shared<Shape>(positions, normals, texCoords, VBO::GEOMETRY_LAYOUT::LAYOUT_TRIANGLES);
