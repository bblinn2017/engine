...
void View::initializeGL()
{
    ...
    m_graphics->addMaterial("grass",mySecondMaterial);


    // TODO (Warmup 1): Initialize application
    m_app->setGraphics(m_graphics);
    m_app->initializeCamera();
}
...
void View::resizeGL(int w, int h)
{
    ...
    m_app->setScreenSize(w,h);
}

void View::mousePressEvent(QMouseEvent *event)
{
    // TODO (Warmup 1): Handle mouse press events
    m_app->getInput()->mousePressEvent(event);
}
...
void View::mouseMoveEvent(QMouseEvent *event)
{
    ...
    QCursor::setPos(mapToGlobal(QPoint(width() / 2, height() / 2)));
    m_app->getInput()->mouseMoveEvent(event);
}

void View::mouseReleaseEvent(QMouseEvent *event)
{
    // TODO (Warmup 1): Handle mouse release here
    m_app->getInput()->mouseReleaseEvent(event);
}

void View::wheelEvent(QWheelEvent *event)
{
    // TODO (Warmup 1): Handle mouse wheel events here
    m_app->getInput()->wheelEvent(event);
}
...
void View::keyReleaseEvent(QKeyEvent *event)
{
    ...
    m_app->getInput()->keyReleaseEvent(event);
}
